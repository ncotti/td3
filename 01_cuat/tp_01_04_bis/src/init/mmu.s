...
/// @brief: Completes pte1 and pte2 corresponding to the VMA, using default
///     values for attributes. This function is a copy of the
///     mmu_fill_page_tables, but with less things.
/// @param r0 (vma): Virtual memory address.
.section .init
.global mmu_fill_pte_from_vma
.type mmu_fill_pte_from_vma, %function
vma             .req r0
pte1_index      .req r1
pte2_index      .req r2
table2_addr     .req r3
...
pte1            .req r7
table2_qtty     .req r8
mmu_fill_pte_from_vma:
    push {r0-r8, lr}
    GET_PTE1_INDEX_FROM_VMA pte1_index, vma
    GET_PTE2_INDEX_FROM_VMA pte2_index, vma

    READ_TTBR0 pte1_addr
    add pte1_addr, pte1_addr, pte1_index, lsl #2
    ldr pte1, [pte1_addr]
    teq pte1, #0
    beq pte1_empty
    b pte1_full
    ...
    pte1_ready:
    add pte2_addr, table2_addr, pte2_index, lsl #2

    push {pte2_addr}
    bl mmu_assign_free_pa
    mov r1, #AP_RW_RW
    mov r2, #MEM_STRONGLY_ORDERED
    mov r3, #G_S_X
    bl mmu_fill_pte2
    pop {pte2_addr}
