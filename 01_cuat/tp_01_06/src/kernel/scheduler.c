...
SYSCALL_SECTION void scheduler_init(void) {
    SYSCALL0(SYSCALL_SCHEDULER_INIT);
}
...
void __scheduler_init(void) {
    ...
    for (i = 0; i < SCHEDULER_MAX_TASKS; i++) {
        ...
        prv_task_contexts[i].asid = i + 1;  // ASID 0 is reserved
    }
}
...
SYSCALL_SECTION uint8_t scheduler_add_task(void(*entrypoint)(void),
    uint32_t* stack_top, uint32_t* table1_addr) {
    return (uint32_t) SYSCALL3(entrypoint, stack_top, table1_addr,
        SYSCALL_SCHEDULER_ADD_TASK);
}
...
SYSCALL_SECTION void scheduler_start(void) {
    SYSCALL0(SYSCALL_SCHEDULER_START);
}

void __scheduler_start(void) {
    __timer_init(TIMER0, SCHEDULER_TIME_PER_TASK, TIMER_MODE_PERIODIC, TRUE, prv_scheduler_change_task);
    #ifdef ENABLE_NEON
    neon_disable();
    #endif // ENABLE_NEON
    __gic_set_swi();   // The first context switch occurs in the IRQ_handler
    prv_idle_task();
}
...
static void prv_scheduler_change_task (void) {
    __timer_clear_interrupt(TIMER0);
