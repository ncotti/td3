...
******************************************************************************/
static void prv_no_handler(void) {}

static timer_dev timer0 = {
    .regs = TIMER0_BASE,
    .irq_handler = prv_no_handler,
    .int_id = INT_TIMER_0_AND_1,
};
PUBLIC_DATA timer_dev* TIMER0 = &timer0;
...
PUBLIC_DATA timer_dev* TIMER1 = &timer1;
...
PUBLIC_DATA timer_dev* TIMER2 = &timer2;
...
PUBLIC_DATA timer_dev* TIMER3 = &timer3;
...
PUBLIC_DATA timer_dev* TIMER4 = &timer4;
...
PUBLIC_DATA timer_dev* TIMER5 = &timer5;
...
PUBLIC_DATA timer_dev* TIMER6 = &timer6;
...
PUBLIC_DATA timer_dev* TIMER7 = &timer7;
...
static void prv_sysctrl_enable_timer(timer_dev* timer);
...
/// @param irq_handler Pointer to the irq handler. Can be prv_no_handler if not needed.
SYSCALL_SECTION void timer_init(timer_dev* timer, uint32_t value,
        timer_mode mode, bool interrupt, void (*irq_handler)(void)) {
    SYSCALL5(timer, value, mode, interrupt, irq_handler, SYSCALL_TIMER_INIT);
}

void __timer_init(timer_dev* timer, uint32_t value,
        timer_mode mode, bool interrupt, void (*irq_handler)(void)) {
    TIMER_SIZE_32BIT(timer->regs->CONTROL);
    TIMER_PRESCALE_1(timer->regs->CONTROL);
    prv_sysctrl_enable_timer(timer);
    __timer_set_mode(timer, mode);
    __timer_load_us(timer, value);
    __timer_clear_interrupt(timer);
    if (interrupt) {
        __timer_enable_interrupt(timer);
    } else {
        __timer_disable_int(timer);
    }
...
SYSCALL_SECTION void timer_resume(timer_dev* timer) {
    SYSCALL1(timer, SYSCALL_TIMER_RESUME)
}

void __timer_resume(timer_dev* timer) {
    TIMER_ENABLE(timer->regs->CONTROL);
}

/// @brief Pause the timer. The count value will be preserved.
/// @param timer Timer device
SYSCALL_SECTION void timer_pause(timer_dev* timer) {
    SYSCALL1(timer, SYSCALL_TIMER_PAUSE);
}

void __timer_pause(timer_dev* timer) {
...
/// @param timer Timer device.
SYSCALL_SECTION void timer_reset(timer_dev* timer) {
    SYSCALL1(timer, SYSCALL_TIMER_RESET);
}

void __timer_reset(timer_dev* timer) {
    timer->regs->VALUE = timer->regs->LOAD;
    TIMER_ENABLE(timer->regs->CONTROL);
}

/// @brief Set the count for the timer. The timer won't be resumed, and the
///     current count value will be overwritten.
///     Use "timer_reset" or "timer_resume" to start operation.
/// @param timer Timer device.
/// @param value Time in milliseconds.
SYSCALL_SECTION void timer_load_ms(timer_dev* timer, uint32_t value) {
    SYSCALL2(timer, value, SYSCALL_TIMER_LOAD_MS);
}

void __timer_load_ms(timer_dev* timer, uint32_t value) {
    __timer_load_us(timer, value*1000);
}
...
SYSCALL_SECTION void timer_load_us(timer_dev* timer, uint32_t value) {
    SYSCALL2(timer, value, SYSCALL_TIMER_LOAD_US);
}

void __timer_load_us(timer_dev* timer, uint32_t value) {
...
/// @param timer Timer device.
/// @param mode Operation mode.
SYSCALL_SECTION void timer_set_mode(timer_dev* timer, timer_mode mode) {
    SYSCALL2(timer, mode, SYSCALL_TIMER_SET_MODE);
}

void __timer_set_mode(timer_dev* timer, timer_mode mode) {
    switch (mode) {
...
/// @param timer Timer device.
SYSCALL_SECTION void timer_enable_interrupt(timer_dev* timer) {
    SYSCALL1(timer, SYSCALL_TIMER_ENABLE_INTERRUPT);
}

void __timer_enable_interrupt(timer_dev* timer) {
    TIMER_ENABLE_INT(timer->regs->CONTROL);
    __gic_enable_interrupt(timer->int_id);
}

/// @brief Disables interrupts for the timer. It does not clear it.
/// @param timer Timer device.
SYSCALL_SECTION void timer_disable_int(timer_dev* timer) {
    SYSCALL1(timer, SYSCALL_TIMER_DISABLE_INT);
}

void __timer_disable_int(timer_dev* timer) {
    TIMER_DISABLE_INT(timer->regs->CONTROL);
    __gic_disable_interrupt(timer->int_id);
}

/// @brief Clears the interrupt flag. Must be called inside the interrupt handler.
/// @param timer Timer device.
SYSCALL_SECTION void timer_clear_interrupt(timer_dev* timer) {
    SYSCALL1(timer, SYSCALL_TIMER_CLEAR_INTERRUPT);
}

void __timer_clear_interrupt(timer_dev* timer) {
    TIMER_CLEAR_INT(timer->regs->INTCLR);
    __gic_clear_interrupt(timer->int_id);
}

/// @brief Returns the time left in milliseconds, rounded upwards.
/// @param timer Timer device.
/// @return Time left in milliseconds, rounded upwards.
SYSCALL_SECTION uint32_t timer_time_ms(timer_dev* timer) {
    return (uint32_t) SYSCALL1(timer, SYSCALL_TIMER_TIME_MS);
}

uint32_t __timer_time_ms(timer_dev* timer) {
...
SYSCALL_SECTION uint32_t timer_time_us(timer_dev* timer) {
    return (uint32_t) SYSCALL1(timer, SYSCALL_TIMER_TIME_US);
}

uint32_t __timer_time_us(timer_dev* timer) {
...
SYSCALL_SECTION bool timer_interrupted(timer_dev* timer) {
    return (bool) SYSCALL1(timer, SYSCALL_TIMER_INTERRUPTED);
}

bool __timer_interrupted(timer_dev* timer) {
