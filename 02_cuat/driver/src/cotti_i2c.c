...
int cotti_i2c_write(u8 value, u8 reg_address, u8 slave_address) {
    ...
    if (wait_event_interruptible_timeout(wq_write, !g_len, msecs_to_jiffies(TIMEOUT_READ_WRITE)) == 0 ||
        wait_for_completion_interruptible_timeout(&comp_stop, msecs_to_jiffies(TIMEOUT_READ_WRITE)) == 0 ) {
...
int cotti_i2c_read(u8 address, u8 slave_address) {
    ...
    if (wait_for_completion_interruptible_timeout(&comp_read, msecs_to_jiffies(TIMEOUT_READ_WRITE)) == 0 ||
        wait_for_completion_interruptible_timeout(&comp_stop, msecs_to_jiffies(TIMEOUT_READ_WRITE)) == 0) {
